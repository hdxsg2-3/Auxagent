  ...
  <div>
    ...
    <div v-else class="space-y-4">
      <div
        ...
        :key="index"
        class="card p-6 cursor-pointer hover:shadow-xl hover:border-violet-300 transition-all group"
      ...
      >
        <div class="flex items-start justify-between">
          <div class="flex-1">
            ...
            <h3 class="text-lg font-bold text-slate-900 mb-2 group-hover:text-violet-600 transition-colors">{{ result.title }}</h3>
            <p class="text-sm text-slate-500 line-clamp-2">{{ result.summary }}</p>
          </div>
          <ChevronRight class="w-5 h-5 text-slate-300 group-hover:text-violet-500 group-hover:translate-x-1 transition-all flex-shrink-0" />
        </div>
      </div>
    </div>
  </div>
...
const navigateToLegalDetail = inject('navigateToLegalDetail', null)
...
function selectResult(result) {
  if (navigateToLegalDetail) {
    navigateToLegalDetail(result)
  }
}
</script>
